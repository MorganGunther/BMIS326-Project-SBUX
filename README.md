...
Predictors and Target Variables: To extend this analysis into predictive models in Deliverable 2, I identified two target variables tied to my original objectives: Avg_Satisfaction (a constructed score representing overall customer satisfaction) and Continue (whether a customer plans to keep buying from Starbucks). My predictor variables are Membership, VisitFrequency, and Age, since these are the customer characteristics my objectives are built around. Whether membership connects to loyalty, and whether visit frequency or age group relate to satisfaction.
...
Membership_Num and VisitFrequency_Num were both statistically significant predictors of satisfaction (p = 0.023 and 0.017 respectively), while the three age group variables were not (all p-values above 0.05). Both significant coefficients were positive, meaning members and more frequent visitors reported higher satisfaction on average, holding the other variables constant. Age did not show a meaningful relationship with satisfaction in this dataset. A likely explanation for these relationships include, members may report higher satisfaction because they've self-selected into the brand relationship already, or because membership perks (rewards, free items, faster ordering) genuinely improve their experience. Similarly, customers who visit more often may simply be more habituated to and comfortable with the experience, or conversely, people who are already more satisfied may naturally choose to visit more often, meaning this relationship could run in either direction.
...
Avg_Satisfaction was clearly statistically significant (p = 0.001) and strongly positive, meaning higher satisfaction is closely tied to a customer's likelihood of continuing to buy from Starbucks. Membership_Num (p = 0.083) and VisitFrequency_Num (p = 0.060) were both close to the standard 0.05 threshold but did not clear it, suggesting a possible but weaker relationship that this sample size may not be large enough to confirm with full statistical confidence. This relationship makes sense because satisfaction is fundamentally about whether a customer's experience met their expectations, so it follows that a more satisfied customer would be more likely to return. This also suggests loyalty isn't being driven by convenience or habit alone, but by whether the actual experience holds up over time.
